...
loop_ufs=[ls[0], ls[1]]
S = te.ragged_compute((M,), [md,], loop_ufs,
                      lambda ds, rds: tvm.sum(tvm.tir.Cast('int32', rds['k'] < (ds[md] + 1)) *
                                              A[ds[md], rds['k']] * B[rds['k'],],
                                              axis=rds['k'], dimensions = [kd]),
                      name = 'S', reduce_axis_ufs = [('k', luf)], width_uf_lists=None)

O = te.ragged_compute((M,), [md,], loop_ufs, lambda ds: alpha*S[ds[md],], name = 'O', width_uf_lists=None)
...
def schedule_op(O, suffix, cache_write_tensor=None):
    ...
    O_local_m_c, O_local_k = tuple(O_local.op.axis) + tuple(O_local.op.reduce_axis)
    #s[O_local].reorder(O_local_k, O_local_m_c)
    #s[O_local].vectorize(O_local_k)
    """
