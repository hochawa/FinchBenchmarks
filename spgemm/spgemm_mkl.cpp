...
int main(int argc, char **argv) {
	...
	eigen_C.resizeNonZeros(rows_start[n_rows_A]);

	for (int i = 0; i < n_rows_A; ++i) {
		eigen_C.outerIndexPtr()[i] = rows_start[i];
	}
	eigen_C.outerIndexPtr()[n_rows_A] = rows_start[n_rows_A];

	for (int i = 0; i < rows_start[n_rows_A]; ++i) {
		eigen_C.innerIndexPtr()[i] = columns[i];
		eigen_C.valuePtr()[i] = values[i];
	}
